...
async function getMovieDetails() {
  try {
    ...
    document.getElementById("movie-title-text").innerText = "Erro ao carregar os detalhes do filme.";
  }
}
...
async function getCredits() {
  try {
    ...
    if (diretor) {
      ...
      directorEl.innerText = "Diretor não informado";
    }

    if (roteiristas.length > 0) {
      writersEl.innerText = roteiristas.map(p => p.name).join(", ");
    } else {
      writersEl.innerText = "Roteiristas não informados";
    }
    ...
    directorEl.innerText = "Informação indisponível";
    writersEl.innerText = "Informação indisponível";
    castListEl.innerHTML = "<p>Erro ao carregar elenco.</p>";
  }
}
...
async function getReviews() {
  ...
  `;
    ...
    reviewsSectionEl.innerHTML = "<p>Erro ao carregar avaliações.</p>";
  }
}
...
async function getMedia() {
  try {
    ...
    if (!videosRes.ok) throw new Error(`Erro vídeos: ${videosRes.status} ${videosRes.statusText}`);
    if (!imagesRes.ok) throw new Error(`Erro imagens: ${imagesRes.status} ${imagesRes.statusText}`);
    ...
    videosListEl.innerHTML = "<p>Erro ao carregar vídeos.</p>";
    postersListEl.innerHTML = "<p>Erro ao carregar pôsteres.</p>";
    backdropsListEl.innerHTML = "<p>Erro ao carregar imagens de fundo.</p>";
  }
}
...
async function getRecommendations() {
  try {
    ...
    section.classList.add("background-recommendations");

    section.innerHTML = `
      <div class="container position-relative">
        <h2 class="recommendations-title">Recomendações</h2>
    ...
    data.results.slice(0, 6).forEach(movie => {
      ...
      const cardHTML = `
        <div class="recommendation-card-content">
          ...
          <div>
            <p class="mb-0 movie-title-section">${movie.title}</p>
            <small>${rating}%</small>
          </div>
        </div>
      `;
